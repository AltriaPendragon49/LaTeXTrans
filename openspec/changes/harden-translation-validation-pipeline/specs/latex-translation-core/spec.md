...
### Requirement: CJK-Aware Engine Fallback

The system SHALL ONLY use CJK-compatible engines (`lualatex`, `xelatex`) for documents identified as CJK (Chinese, Japanese, Korean) to prevent document corruption.

#### Scenario: Disabling pdflatex for Japanese translation
- **WHEN** `target_language` is "ja"
- **AND** primary engines fail
- **THEN** the system SHALL NOT attempt `pdflatex` compilation.
