...
- `st.progress()` - 进度条显示
- `st.text()` / `st.status_text` - 状态文本显示  
- `st.spinner()` - 加载动画
- `st.success()` / `st.error()` - 成功/错误消息
- `sys.stderr = open(os.devnull, 'w')` / `sys.stderr = sys.__stderr__` - 用于抑制 Streamlit 输出
...
| 单编译器 | 智能回退编译器 ✅ |
| `tex source/` | `data/uploads/{task_id}/` |
| `outputs/` | `data/outputs/{task_id}/` |
