...
- Newer systems remain in the codebase for future use, but are not invoked in parity tasks.

## Non-Goals
- Do not delete hard-freeze, repair, diagnostics, structure guard, or fallback code.
...
5. Unused modern systems
   - Existing modern modules remain present.
   - Parity task config and task logs must make clear that these systems are not invoked: hard-freeze, structure guard, controlled repair, ultimate downgrade, post-compile target-language fallback, compile diagnostics, residual-English fallback, parser environment LLM judgment, intelligent compiler fallback, RAG or terminology mutation, and any backend-only quality improvement path.
...
7. Precedence over existing modern specs
   ...
   - Those existing systems can still apply only to future non-parity modes introduced by a separate approved spec.
