...
## 1. 代码审计和重构准备
- [ ] 1.1 回顾`prototype_system/src/agents/coordinator_agent.py`了解工作流程编排
- [ ] 1.2 回顾`prototype_system/src/formats/latex/parser.py`和`compile.py`用于 AST 解析和编译逻辑
- [ ] 1.3 回顾`prototype_system/src/formats/latex/utils.py`用于 arXiv 下载实用程序
- [ ] 1.4 记录可重用组件和所需的修改（删除 Streamlit 依赖项）
- [ ] 1.5 创建重用映射文档列出要复制/改编的文件

## 2. 项目结构初始化（仅后端）
- [ ] 2.1 创建`backend/app/`目录结构（`core/`、`api/routes/`、`services/`）
- [ ] 2.2 创建`data/`目录结构（`uploads/`、`outputs/`、`terms/`）
- [ ] 2.3 创建`docker/`未来容器化的目录
- [ ] 2.4 初始化`backend/requirements.txt`具有 FastAPI 依赖项

**注意**：前端初始化（React/Vite 设置）在`add-web-mvp-frontend`改变。

## 3.后端核心服务
- [ ] 3.1 将 LaTeX 解析器从原型调整为 `backend/app/services/latex/parser.py`
  - 删除`st.progress()`和`st.text()`依赖关系
  - 添加Web集成进度回调机制
- [ ] 3.2 将 LaTeX 实用程序调整为 `backend/app/services/latex/utils.py`
  - 复制`batch_download_arxiv_tex()`及相关功能
  - 调整文件路径以适应网络环境
- [ ] 3.3 实现带回退功能的智能 LaTeX 编译器 (`backend/app/services/latex/compiler.py`)
  - 创建`compile_with_fallback()`首先尝试 pdflatex，然后尝试 xelatex 的函数
  - 实施`.log`文件解析器对错误进行计数（匹配模式：`!LaTeX Error`、`!未定义的控制序列`、`!Missing`）
  - 比较错误计数并选择错误较少的 PDF
  - 返回最佳 PDF 或在两者均失败时引发异常
  - 支持 MiKTeX 自动安装缺少的软件包
- [ ] 3.4 代理系统适配 `backend/app/services/agents/`
  - 复制“coordinator_agent.py”、“parser_agent.py”、“translator_agent.py”等。
  - 用结构化日志记录替换 Streamlit 日志记录 (Python`logging`模块）
  - 更新`generator_agent.py`使用新的`compile_with_fallback()`功能
- [ ] 3.5 创建任务管理器服务(`backend/app/services/task_manager.py`)
  - 内存中任务状态跟踪
  - 进度更新机制（0-100%）
  - 任务状态管理（待处理→处理→完成/失败）
...
## 5. 集成与配置
- [ ] 5.1 创建后端配置模块(`backend/app/core/config.py`)
  - 从环境变量或“config/default”加载设置。托姆尔`
  - 使用特定参数配置LLM API：
...
## 6. 后端API测试
- [ ] 6.1 测试`POST /upload`带样本的终点`.tex`通过curl/Postman 文件
...
- [ ] 8.2 创建后端启动脚本：
  - `backend/start.sh`（设置环境，运行 uvicorn）
- [ ] 8.3 记录后端环境设置要求
