...
> [!NOTE]
> **拖拽上传功能** 已在 `add-advanced-config-temp-user` 中实现，本变更直接复用。
...
flowchart TD
    ...
    H --> I[在本地创建文件目录]
    I --> J[启动翻译 Agent]
    J -->|更新状态| K[更新任务状态到 Supabase]
```

## 新建翻译请求（正式用户特有）

> [!IMPORTANT]
> 核心概念：**访客任务 = 临时 task，正式用户任务 = 持久 task**
