...
### 3.2 编译自愈机制（Compiler Agent）

#### [ ] 步骤 1：创建 MiKTeX Docker 镜像
**文件**：`docker/Dockerfile.miktex`
...
RUN apt-get update && \
    ...
    wget https://miktex.org/download/ubuntu/pool/main/m/miktex/miktex_22.1_amd64.deb && \
    dpkg -i miktex_22.1_amd64.deb

# 启用 install on the fly
...
**构建镜像**：
```bash
docker build -t latextrans-miktex -f docker/Dockerfile.miktex .
```

#### [ ] 步骤 2：Python 调用 subprocess 运行 xelatex
**文件**：`backend/services/latex/compile.py`（适配 Docker）

```python
import subprocess

def compile_in_docker(tex_file, output_dir):
    cmd = [
        "docker", "run", "--rm",
        "-v", f"{tex_file}:/work/main.tex",
        "-v", f"{output_dir}:/work/output",
        "latextrans-miktex",
        "xelatex", "-output-directory=/work/output", "/work/main.tex"
    ]
    result = subprocess.run(cmd, capture_output=True, timeout=600)
    return result.returncode == 0
```

#### [ ] 步骤 3：编写正则解析 `.log` 文件错误
**文件**：`backend/services/latex/log_parser.py`
...
#### [ ] 步骤 4：实现自愈循环
**文件**：`backend/services/agents/compiler_agent.py`

```python
async def compile_with_self_healing(tex_file, max_retries=3):
    for i in range(max_retries):
        success = compile_in_docker(tex_file, output_dir)
        if success:
            return True

        # 解析错误
        errors = parse_latex_log(f"{output_dir}/main.log")
...
**LLM 修复 Prompt**：
```python
def llm_fix_latex(tex_content, errors):
    prompt = f"""
    The following LaTeX file has compilation errors:
    ...
    Please fix the errors and return the corrected LaTeX code.
    """
