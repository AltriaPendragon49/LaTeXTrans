...
def test_legacy_validate_does_not_block_on_residual_english_completeness(
    monkeypatch,
    tmp_path,
):
    from backend.app.services.agents import langgraph_orchestrator as orch
            ...
            return [
                {
                    ...
                    "num_or_ph": "1",
                    "error_type": "C1",
                    "completeness_error": "long_english_prose_span",
                }
            ]
    ...
    assert result["residual_english_requires_fallback"] is False
...
def test_legacy_route_after_validate_skips_residual_english_fallback():
    from backend.app.services.agents import langgraph_orchestrator as orch
...
def test_legacy_route_after_generate_does_not_retry_post_compile_fallback():
    from backend.app.services.agents import langgraph_orchestrator as orch
    from backend.app.services.agents.pipeline_schema import FallbackReport
    ...
    assert orch._route_after_generate(state) == "abort_structure_invalid"
