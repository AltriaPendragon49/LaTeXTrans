...
def test_raw_cache_builds_pdf_and_eprint_keys() -> None:
    ...
    assert arxiv_raw_cache.raw_pdf_object_key("2501.12345", settings=settings) == "arxiv/raw/pdf/2501.12345.pdf"
    assert arxiv_raw_cache.raw_eprint_object_key("2501.12345", settings=settings) == "arxiv/raw/e-print/2501.12345"
    assert arxiv_raw_cache.raw_pdf_object_key("hep-th/9901001", settings=settings) == "arxiv/raw/pdf/hep-th/9901001.pdf"


def test_raw_cache_default_keys_match_arxiv_origin_paths() -> None:
    settings = _settings(arxiv_raw_cache_prefix="")

    assert arxiv_raw_cache.raw_pdf_object_key("2501.12345", settings=settings) == "pdf/2501.12345.pdf"
...
def test_raw_cache_builds_signed_pdf_url_with_inline_headers() -> None:
    ...
    assert url == "https://cos.example.com/arxiv/raw/pdf/2501.12345.pdf?sign=abc"
    assert backend.calls == [
        {
            "object_key": "arxiv/raw/pdf/2501.12345.pdf",
            "expires_in": 900,
            "params": {
...
def test_raw_cache_constructs_cos_backend_without_business_base_prefix(monkeypatch) -> None:
    ...
    assert url == "https://cos.example.com/pdf/2501.12345.pdf?sign=abc"
